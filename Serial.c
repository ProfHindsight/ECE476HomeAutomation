...
void SER_init (int uart_port, int baudrate) 
{
  ...
  if (uart_port == 0) {                          // UART0 
    ...
    pUart = (LPC_UART_TypeDef *)LPC_UART0;
		LPC_UART0->IER = 1;														//Enable the receive data interrupt
		//Since no FIFO enabled, this should clear any pending RDA interrupt
		LPC_UART0->RBR;
		//Clear pending and enable the NVIC IRQ
    ...
    pUart = (LPC_UART_TypeDef *)LPC_UART2;
		pUart->IER = 1;														//Enable the receive data interrupt
		//Since no FIFO enabled, this should clear any pending RDA interrupt
		pUart->RBR;
		//Clear pending and enable the NVIC IRQ
  ...
  pUart->FCR = 0x07;                             // Enable and reset TX and RX FIFO
	//By default, interrupt when 1 character in FIFO.
}
